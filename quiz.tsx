...
export const QuizPage = ({ navigateTo }) => {
  ...
  /**
   * Hybrid Approach:
   * 1. Call Backend for Recommendations.
   * 2. Call Gemini for Analysis & Match Scores based on those recommendations.
   */
  const getCareerSuggestions = async (finalAnswers) => {
    ...
    try {
      // 1. Call Backend
      const backendResp = await fetch(`${BACKEND_BASE_URL}/api/quiz/analyze`, {
        method: 'POST',
      ...
      if (!backendResp.ok) {
          // Only throw if backend fails, otherwise proceed to parsing
          throw new Error(`Backend error: ${backendResp.statusText}`);
      }
      
      const backendData = await backendResp.json();

      // Robust parsing of backend data (handling diverse potential structures)
      let rawRecs = [];
      if (backendData.results && Array.isArray(backendData.results)) {
        rawRecs = backendData.results;
      } else if (backendData.recommendations && Array.isArray(backendData.recommendations)) {
        rawRecs = backendData.recommendations;
      } else if (Array.isArray(backendData)) {
        rawRecs = backendData;
      } else {
         // Last resort: check for embedded JSON in raw_response string
         const rawText = typeof backendData === 'string' ? backendData : (backendData.raw_response || "");
         const arrayMatch = rawText.match(/\[\s*\{[\s\S]*?\}\s*\]/);
         if (arrayMatch) {
             try { rawRecs = JSON.parse(arrayMatch[0]); } catch(e) {}
         }
      }

      if (!rawRecs || rawRecs.length === 0) {
          throw new Error("No recommendations found from backend.");
      }

      const recommendations = rawRecs.map(r => ({
          careerName: r.careerName || r.title || r.careerTitle || "Unknown Career",
          reason: r.reason || r.description || r.explanation || r.details || "Good fit based on answers."
      })).slice(0, 5); // Take top 5

      // 2. Call Gemini for Analysis & Scores
      const careerList = recommendations.map(r => r.careerName).join(', ');
      
      const prompt = `
        User Profile based on Quiz Answers:
        ${finalAnswers.map((a, i) => `Q${i+1}: ${a.question} A: ${a.answer}`).join('\n')}

        The system has recommended these careers: ${careerList}.

        Task:
        1. Write a concise "analysis" (2-3 sentences) of the user's personality/strengths based on their answers.
        2. For EACH recommended career listed above, assign a "matchScore" (0-100) indicating fit quality.
      `;
      ...
      // 3. Merge Data
      const mergedResults = recommendations.map(rec => {
          // Fuzzy match to associate Gemini scores with Backend careers
          const scoreData = geminiData.scores?.find(s => 
              s.careerName.toLowerCase().includes(rec.careerName.toLowerCase()) || 
              rec.careerName.toLowerCase().includes(s.careerName.toLowerCase())
          );
          
          return {
              ...rec,
              matchScore: scoreData ? scoreData.matchScore : 85 // Default score if matching fails
          };
      });

      setQuizResults(mergedResults);
      setQuizAnalysis(geminiData.analysis);
      setQuizStage('results');

    } catch (err) {
      console.error("Quiz generation error:", err);
      setError("Sorry, we encountered an issue generating your results. Please try again.");
      setQuizStage('results');
    }
  };
    ...
    <div className="quiz-page-container">
      ...
      {quizStage === 'results' && (
        <div className="quiz-results-view">
          <h2 className="quiz-results-title">Your Career Profile</h2>
          
          {error && <div className="error-message">{error}</div>}

          {quizAnalysis && (
            <div className="analysis-summary-card" style={{
                background: 'white',
                padding: '2rem',
                borderRadius: '12px',
                ...
                <h3 style={{marginTop: 0, color: '#1f2937', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.4rem'}}>
                    <span>📊</span> Analysis
                </h3>
                <p style={{fontSize: '1.1rem', lineHeight: '1.6', color: '#4b5563', margin: '1rem 0 0 0'}}>{quizAnalysis}</p>
            </div>
          )}

          <h3 className="quiz-results-subtitle" style={{marginBottom: '1.5rem', color: '#4b5563'}}>Recommended Careers</h3>
              ...
              {quizResults.map((result, index) => (
                <div key={index} className="quiz-result-card">
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: '0.5rem'}}>
                      <h4 style={{margin:0}}>{result.careerName}</h4>
                      <span className="match-badge">{result.matchScore}% Match</span>
                  </div>
                  <div className="match-bar-container">
                      <div className="match-bar-fill" style={{width: `${result.matchScore}%`}}></div>
                  </div>
                  <p>{result.reason}</p>
                </div>
              ))}
