  ...
  useEffect(() => {
    const init = async () => {
        if (course && user) {
            ...
            try {
                // Using v8 syntax: db.collection().doc().collection().doc()
                const wishlistRef = db.collection("users").doc(user.uid).collection("Courses").doc("Wishlist");
      ...
      try {
          if (isWishlisted) {
              ...
              await wishlistRef.update({
                  wishlist_course: firebase.firestore.FieldValue.arrayRemove(course.name)
              });
              setIsWishlisted(false);
          } else {
              // Add to wishlist array (create doc if needed using set with merge)
              await wishlistRef.set({
                  wishlist_course: firebase.firestore.FieldValue.arrayUnion(course.name),
